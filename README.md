...
8. [Локальная сеть (телефон и другие устройства)](#локальная-сеть-телефон-и-другие-устройства)
9. [Быстрый старт (Docker)](#быстрый-старт-docker)
10. [Деплой на сервер](#деплой-на-сервер)
11. [API и авторизация](#api-и-авторизация)
...
## Локальная сеть (телефон и другие устройства)

Скрипт поднимает Docker-стенд, при занятости портов подбирает свободные, определяет IP текущей сети и печатает рабочую ссылку.
...
```powershell
python scripts\lan_up.py
```

После запуска в консоли будет блок вида:

```
Рабочая ссылка:  http://192.168.0.99:8080/
Админка:         http://192.168.0.99:8080/admin.html
```

Откройте эту ссылку на телефоне. Устройство должно быть в **той же Wi‑Fi / LAN**, что и компьютер (не мобильный интернет). Логин админа: `admin` / `demo123456`.

| Флаг | Зачем |
|------|--------|
| `--no-build` | Быстрый повторный запуск без пересборки образов |
| `--port 8080` | Предпочитаемый порт витрины, если 8080 занят — скрипт возьмёт соседний |
| `--print-only` | Только напечатать ссылку, Docker не трогать |

Если телефон не открывает сайт, разрешите входящий TCP-порт витрины в брандмауэре (скрипт пытается сделать это сам, на Windows часто нужны права администратора).
...
| auth | 8004 |
| assets | 8005 |
| PostgreSQL | 5432 (если занят, `scripts/lan_up.py` берёт соседний) |
| MinIO console | 9001 |
